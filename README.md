<!-- Profile Banner -->
![header](https://capsule-render.vercel.app/api?type=waving&color=0:00c6ff,100:0072ff&height=220&section=header&text=Chandika%20Lakshmi%20Sriram&fontSize=38&fontColor=ffffff)

![Typing SVG](https://readme-typing-svg.demolab.com/?lines=Hey+I'm+Chandika+Lakshmi;DevOps+%26+Cloud+Engineer;AWS+%7C+CI%2FCD+%7C+Automation;Learning+in+Public&center=true&width=800)

---

### 🚀 DevOps & Cloud Engineer | AWS | CI/CD | Automation | Python

![Profile Views](https://komarev.com/ghpvc/?username=sriramch163&style=flat-square&color=blue)
![Open to Work](https://img.shields.io/badge/Open%20To%20Work-Yes-success)
![Focus](https://img.shields.io/badge/Focus-DevOps%20%7C%20Cloud-blue)

---

## 👋 About Me

- Aspiring **DevOps & Cloud Engineer** from India  
- Focused on **automation, scalability, and reliability**
- Hands-on with **AWS, Linux, CI/CD pipelines**
- Actively building projects and learning in public  
- Mission: **Automate Everything. Break Nothing.**

---

## 🧠 Skills & Tech Stack

### ☁️ Cloud & Infrastructure
AWS (EC2, S3, IAM, VPC, RDS, CloudWatch)  
Terraform | CloudFormation

### 🔁 DevOps & Automation
Jenkins | GitHub Actions | Docker | CI/CD

### 💻 Programming
Python | Bash | Shell Scripting

### 🗄️ Databases
MySQL | DynamoDB

---

## 📌 Projects & Domains

| Domain | Tech Stack | Project |
|------|-----------|--------|
| AWS Cloud | EC2, S3, IAM | AWS Hands-on Projects |
| CI/CD | Jenkins, GitHub Actions | CI/CD Pipeline Automation |
| Serverless | Lambda, API Gateway | AWS Serverless Projects |
| Containers | Docker | Dockerized Applications |

---

## 📊 GitHub Analytics

![GitHub Stats](https://github-readme-stats.vercel.app/api?username=sriramch163&show_icons=true&theme=radical)
![GitHub Streak](https://streak-stats.demolab.com/?user=sriramch163&theme=dark&hide_border=true)


---

## 🌐 Let's Connect

[![LinkedIn](https://img.shields.io/badge/LinkedIn-blue?logo=linkedin)](https://linkedin.com)
[![GitHub](https://img.shields.io/badge/GitHub-black?logo=github)](https://github.com/sriramch163)
[![Gmail](https://img.shields.io/badge/Gmail-red?logo=gmail)](mailto:yourmail@gmail.com)

---

![Snake animation](https://github.com/sriramch163/sriramch163/blob/output/github-contribution-grid-snake.svg)


### ✨ “Code. Automate. Deploy. Repeat.”
